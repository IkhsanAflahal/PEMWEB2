...
$sqlKelurahan = "SELECT id, nama_kelurahan FROM klurahan";
...
<div class="container mt-5">
  <form method="POST" action="proses_pasien.php">
    ...
    <div class="form-group row">
      <label for="klurahan_id" class="col-4 col-form-label">Kelurahan</label>
      <div class="col-8">
        <select id="klurahan_id" name="klurahan_id" class="form-control">
          <option value="">-- Pilih Kelurahan --</option>
          <?php foreach($rs as $row): ?>
            <option value="<?= htmlspecialchars($row['id']) ?>" <?= (isset($patientData['klurahan_id']) && $patientData['klurahan_id'] == $row['id']) ? 'selected' : '' ?>>
